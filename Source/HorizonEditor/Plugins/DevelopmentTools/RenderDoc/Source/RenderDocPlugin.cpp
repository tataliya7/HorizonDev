...
namespace Horizon
{
    ...
    void RenderDocPluginInit()
    {
        ...
        bool attachOnStartup = configuration.Get("RenderDocPluginSettings.AttachOnStartup").AsBooleanOr(false);
        if (attachOnStartup)
        {
            std::string renderDocPath = configuration.Get("RenderDocPluginSettings.RenderDocPath").AsStringOr("");
