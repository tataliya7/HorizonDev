...
namespace Horizon::USDImporter
{
    ...
    void USDImportStart(USDImportTaskData* data)
    {
        ...
        if (data->settings.importSkeletons)
        {
            stageImporter->ImportAllSkeletons();
        }

        // Create entities
        for (USDPrimImporter* importer : stageImporter->GetPrimImporters())
        {
            if (!importer)
            {
                continue;
            ...
            importer->CreateEntity(data->scene);
        }
        ...
        for (USDPrimImporter* importer : stageImporter->GetPrimImporters())
        {
            ...
            importer->AddComponents(data->scene);

            USDPrimImporter* parent = importer->GetParent();
            if (!parent)
            {
