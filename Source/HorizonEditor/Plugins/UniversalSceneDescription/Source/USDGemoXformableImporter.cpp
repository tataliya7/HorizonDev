...
namespace Horizon::USDImporter
{
    ...
    void USDGemoXformableImporter::GetLocalTransformation(Matrix4x4f* transform, float time, float scale)
    {
        if (!geomXformable)
        {
            return;
        ...
        *transform = UsdToHorizon::ConvertMatrix(pxrMat4f);
        ...
        if (IsRoot() && (scale != 1.0))
        {
            Matrix4x4f scaleMatrix = Math::ScaleMatrix(Vector3f(scale, scale, scale));
