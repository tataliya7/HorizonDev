...
namespace Horizon::USDImporter
{
    ...
    void USDStageImporter::ImportAllMaterials()
    {
        ...
        for (const std::string& materialPath : importContext.materialPaths)
        {
            ...
            if (importContext.materialMap.contains(materialPath))
            {
                continue;
            }
    ...
    void USDStageImporter::ImportAllSkeletons()
    {
        if (!IsValid())
        {
            return;
        ...
        USDSkelSkeletonImporter skeletonImporter(importContext);
        for (const std::string& skeletonPath : importContext.skeletonPaths)
        {
            pxr::UsdPrim prim = stage->GetPrimAtPath(pxr::SdfPath(skeletonPath));
            pxr::UsdSkelSkeleton usdSkelSkeleton(prim);
            if (!usdSkelSkeleton)
            {
                continue;
            }

            printf("Skeleton Path: %s\n", skeletonPath.c_str());

            if (importContext.skeletons.contains(skeletonPath))
            {
                continue;
            }

            Skeleton* skeleton = skeletonImporter.ImportSkeleton(usdSkelSkeleton);
            importContext.skeletons[skeletonPath] = skeleton;
        }
    }
    ...
    USDPrimImporter* USDStageImporter::CreatePrimImporterImpl(const pxr::UsdPrim& prim)
    {
        ...
        if (importSettings.importMaterials && prim.IsA<pxr::UsdShadeMaterial>())
        {
        ...
        {
            importContext.skeletonPaths.push_back(prim.GetPath().GetAsString());
            return nullptr;
        }
        ...
        {
            return new USDSkelRootImporter(importContext, prim);
        }
