...
namespace Horizon
{
    uint32 ScreenSpaceLightShaftsDownsampleFactor = 2;
    uint32 ScreenSpaceLightShaftsRadialBlurPassCount = 3;
        ...
        const SceneView& view)
    {
        ...
        renderGraph.AddPass(std::format("LightShaftsDownsample (Compute, {}x{})", lightShaftsTextureSize.width, lightShaftsTextureSize.height), RenderGraphPassFlags::Compute,
            [&](RenderGraphBuilder& builder)
            {
                ...
                {
                    uint32 groupCountX = ComputeWorkGroupCount(lightShaftsTextureSize.width, 8);
                    uint32 groupCountY = ComputeWorkGroupCount(lightShaftsTextureSize.height, 8);
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                    {
                        ...
                        RenderBackendShaderProgramHandle graphicsShader = shaderLibrary->GetShaderProgramHandle(ShaderID::LightShaftsApply);
