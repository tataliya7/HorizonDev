...
namespace Horizon
{
        ...
        RenderGraphTextureHandle autoExposureTexture)
    {
        ...
        uint32 width = std::max(1u, ComputeWorkGroupCount(targetResolutionX, downsampleFactor));
        uint32 height = std::max(1u, ComputeWorkGroupCount(targetResolutionY, downsampleFactor));
        ...
        renderGraph.AddPass(std::format("LocalExposureComputeLuminances (Compute, {}x{})", width, height), RenderGraphPassFlags::Compute,
            [&](RenderGraphBuilder& builder)
            {
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                    {
                        uint32 groupCountX = ComputeWorkGroupCount(width, PostProcessingThreadGroupCountX);
                        uint32 groupCountY = ComputeWorkGroupCount(height, PostProcessingThreadGroupCountY);
                        ...
                        RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle(ShaderID::LocalExposureComputeLuminances);
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                    {
                        ...
                        RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle(ShaderID::LocalExposureComputeWeights);
                        commandList.Dispatch2D(
                            computeShader,
                            shaderArguments,
                            groupCountX,
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                    {
                        ...
                        for (uint32 mipLevel = 1; mipLevel < mipLevels; mipLevel++)
                        {
                            ...
                            uint32 groupCountX = ComputeWorkGroupCount(w, 8);
                            uint32 groupCountY = ComputeWorkGroupCount(h, 8);
        ...
        renderGraph.AddPass(std::format("LocalExposureBlendExposures (Compute, {}x{})", coarsestMipLevelWidth, coarsestMipLevelHeight), RenderGraphPassFlags::Compute,
            [&](RenderGraphBuilder& builder)
            {
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                    {
                        uint32 groupCountX = ComputeWorkGroupCount(coarsestMipLevelWidth, PostProcessingThreadGroupCountX);
                        uint32 groupCountY = ComputeWorkGroupCount(coarsestMipLevelHeight, PostProcessingThreadGroupCountY);
                        ...
                        RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle(ShaderID::LocalExposureBlendExposures);
                        commandList.Dispatch2D(
                            computeShader,
                            shaderArguments,
                            groupCountX,
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                    {
                        ...
                        for (int32 mipLevel = coarsestMipLevel; mipLevel > displayMipLevel; mipLevel--)
                        {
                            ...
                            uint32 groupCountX = ComputeWorkGroupCount(w, PostProcessingThreadGroupCountX);
                            uint32 groupCountY = ComputeWorkGroupCount(h, PostProcessingThreadGroupCountY);
                            ...
                            RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle(ShaderID::LocalExposureBlendLaplacian);
                            commandList.Dispatch2D(
                                computeShader,
                                shaderArguments,
                                groupCountX,
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                    {
                        ...
                        RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle(ShaderID::LocalExposureGuidedUpsampling);
                        commandList.Dispatch2D(
                            computeShader,
                            shaderArguments,
                            groupCountX,
