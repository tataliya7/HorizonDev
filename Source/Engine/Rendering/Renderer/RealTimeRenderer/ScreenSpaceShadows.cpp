            ...
            for (uint32 cascadeIndex = 0; cascadeIndex < numDynamicShadowCascades; cascadeIndex++)
            {
                renderGraph.AddPass("CascadedShadowMap", RenderGraphPassFlags::Graphics,
                    [&](RenderGraphBuilder& builder)
                    {
                        ...
                        builder.BindDepthTarget(shadowMap, RenderBackendRenderTargetLoadOp::Clear, RenderBackendRenderTargetStoreOp::Store, 0, cascadeIndex);
                        ...
                        return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                        {
                            ...
                            RenderBackendShaderHandle graphicsShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::CascadedShadowMap);
        ...
        renderGraph.AddPass("ScreenSpaceShadows", RenderGraphPassFlags::Compute,
            [&](RenderGraphBuilder& builder)
            {
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                {
                    ...
                    shaderArguments.BindTextureUAV(5, RenderBackendTextureUAVDesc::Create(registry.GetRenderBackendTexture(screenSpaceShadowMaskTexture), 0));

                    RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::ScreenSpaceShadowsDirectionalLight);
