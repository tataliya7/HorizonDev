        ...
        RenderGraphTextureHandle sceneColorTexture)
    {
        ...
        renderGraph.AddPass(std::format("EditorSelectionOutlineMaskGen (Graphics, {}x{})", targetResolutionX, targetResolutionY), RenderGraphPassFlags::Graphics,
            [&](RenderGraphBuilder& builder)
            {
                ...
                builder.BindColorTarget(0, maskTexture, RenderBackendRenderTargetLoadOp::Clear, RenderBackendRenderTargetStoreOp::Store);
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                {
                    ...
                    for (const auto& drawCallInfo : renderEngine->drawList)
                    {
                        ...
                        RenderBackendShaderHandle graphicsShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::EditorSelectionOutlineMaskGen);
        ...
        renderGraph.AddPass(std::format("EditorSelectionOutlineSetup (Compute, {}x{})", targetResolutionX, targetResolutionY), RenderGraphPassFlags::Compute,
            [&](RenderGraphBuilder& builder)
            {
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                {
                    ...
                    RenderBackendShaderArguments shaderArguments = {};
                    shaderArguments.BindBuffer(0, sceneViewShaderParametersBuffer, 0);
                    shaderArguments.BindTextureSRV(1, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(maskTexture)));
                    shaderArguments.BindTextureUAV(2, RenderBackendTextureUAVDesc::Create(registry.GetRenderBackendTexture(jumpFloodTexture0), 0));

                    RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::EditorSelectionOutlineSetup);
            ...
            renderGraph.AddPass(std::format("EditorSelectionOutlineJumpFlood-Step{} (Compute, {}x{}, StepWidth={})", step, targetResolutionX, targetResolutionY, stepWidth), RenderGraphPassFlags::Compute,
                [&](RenderGraphBuilder& builder)
                {
                    ...
                    return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                    {
                        ...
                        RenderBackendShaderArguments shaderArguments = {};
                        shaderArguments.BindBuffer(0, sceneViewShaderParametersBuffer, 0);
                        shaderArguments.BindTextureSRV(1, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(jumpFloodPassInputTexture)));
                        shaderArguments.BindTextureUAV(2, RenderBackendTextureUAVDesc::Create(registry.GetRenderBackendTexture(jumpFloodPassOutputTexture), 0));
                        shaderArguments.PushConstants(0, (float)stepWidth);

                        RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::EditorSelectionOutlineJumpFlood);
                        commandList.Dispatch2D(
                            computeShader,
                            shaderArguments,
                            dispatchX,
        ...
        renderGraph.AddPass(std::format("EditorSelectionOutlineComposite (Compute, {}x{})", targetResolutionX, targetResolutionY), RenderGraphPassFlags::Compute,
            [&](RenderGraphBuilder& builder)
            {
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                {
                    ...
                    RenderBackendShaderArguments shaderArguments = {};
                    shaderArguments.BindBuffer(0, sceneViewShaderParametersBuffer, 0);
                    shaderArguments.BindTextureSRV(1, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(jumpFloodTexture)));
                    shaderArguments.BindTextureUAV(2, RenderBackendTextureUAVDesc::Create(registry.GetRenderBackendTexture(outputTexture), 0));

                    RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::EditorSelectionOutlineComposite);
                    commandList.Dispatch2D(
                        computeShader,
                        shaderArguments,
                        dispatchX,
