        ...
        RenderGraphTextureHandle bloomTexture)
    {
        ...
        renderGraph.AddPass(std::format("LensFlaresGhost (Graphics, {}x{})", lensFlaresTextureWidth, lensFlaresTextureHeight), RenderGraphPassFlags::Graphics,
            [&](RenderGraphBuilder& builder)
            {
                ...
                builder.BindColorTarget(0, lensFlaresGhostTexture, RenderBackendRenderTargetLoadOp::Clear, RenderBackendRenderTargetStoreOp::Store);
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                {
                    ...
                    shaderArguments.BindTextureSRV(1, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(halfResolutionSceneColorTexture)));
                    ...
                    graphicsPipelineState.colorBlendState.targetBlends[0] = additiveRGBColorBlendAttachmentState;

                    RenderBackendShaderHandle graphicsShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::LensFlaresGhost);
                    commandList.Draw(
                        graphicsShader,
        ...
        renderGraph.AddPass(std::format("LensFlaresTileCulling (Compute, {}x{})", lensFlaresTileCullingTextureDesc.width, lensFlaresTileCullingTextureDesc.height), RenderGraphPassFlags::Compute,
            [&](RenderGraphBuilder& builder)
            {
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                {
                    uint32 dispatchX = Math::CeilDiv(lensFlaresTileCullingTextureDesc.width, PostProcessingThreadGroupCountX);
                    uint32 dispatchY = Math::CeilDiv(lensFlaresTileCullingTextureDesc.height, PostProcessingThreadGroupCountY);

                    RenderBackendShaderArguments shaderArguments = {};
                    shaderArguments.BindTextureSRV(0, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(halfResolutionSceneColorTexture)));
                    shaderArguments.BindTextureUAV(1, RenderBackendTextureUAVDesc::Create(registry.GetRenderBackendTexture(tileCullingTexture), 0));

                    RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::LensFlaresTileCulling);
        ...
        renderGraph.AddPass(std::format("LensFlaresGlare (Graphics, {}x{})", lensFlaresGlareTextureDesc.width, lensFlaresGlareTextureDesc.height), RenderGraphPassFlags::Graphics,
            [&](RenderGraphBuilder& builder)
            {
                ...
                builder.BindColorTarget(0, lensFlaresGlareTexture, RenderBackendRenderTargetLoadOp::Clear, RenderBackendRenderTargetStoreOp::Store);
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                {
                    ...
                    shaderArguments.BindTextureSRV(1, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(tileCullingTexture)));
                    shaderArguments.PushConstants(0, (float)lensFlaresTextureWidth);
                    shaderArguments.PushConstants(1, (float)lensFlaresTextureHeight);
                    ...
                    RenderBackendShaderHandle graphicsShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::LensFlaresGlare);
                    commandList.Draw(
                        graphicsShader,
                        graphicsPipelineState,
                        shaderArguments,
        ...
        renderGraph.AddPass(std::format("LensFlaresCombine (Graphics, {}x{})", bloomTextureWidth, bloomTextureHeight), RenderGraphPassFlags::Graphics,
            [&](RenderGraphBuilder& builder)
            {
                ...
                builder.BindColorTarget(0, lensFlaresTexture, RenderBackendRenderTargetLoadOp::Load, RenderBackendRenderTargetStoreOp::Store);
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                {
                    ...
                    shaderArguments.BindTextureSRV(2, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(lensFlaresGhostTexture)));
                    shaderArguments.BindTextureSRV(3, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(lensFlaresGlareTexture)));
                    shaderArguments.BindTextureSRV(4, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(halfResolutionSceneColorTexture)));
                    ...
                    RenderBackendShaderHandle graphicsShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::LensFlaresCombine);
                    commandList.Draw(
                        graphicsShader,
                        graphicsPipelineState,
                        shaderArguments,
