        ...
        RenderGraphTextureHandle sceneColorTexture)
    {
        ...
        renderGraph.AddPass("DepthOfFieldSetup", RenderGraphPassFlags::Compute,
            [&](RenderGraphBuilder& builder)
            {
                ...
                {
                    uint32 dispatchX = Math::CeilDiv(view.targetWidth, PostProcessingThreadGroupCountX);
                    uint32 dispatchY = Math::CeilDiv(view.targetHeight, PostProcessingThreadGroupCountY);
                    ...
                    shaderArguments.BindTextureUAV(3, RenderBackendTextureUAVDesc::Create(registry.GetRenderBackendTexture(depthOfFieldCoCTexture), 0));
                    ...
                    RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::DepthOfFieldSetup);
        ...
        renderGraph.AddPass("DepthOfFieldGather", RenderGraphPassFlags::Compute,
            [&](RenderGraphBuilder& builder)
            {
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                {
                    uint32 dispatchX = Math::CeilDiv(dofWidth, PostProcessingThreadGroupCountX);
                    uint32 dispatchY = Math::CeilDiv(dofHeight, PostProcessingThreadGroupCountY);

                    RenderBackendShaderArguments shaderArguments = {};
                    shaderArguments.BindBuffer(0, sceneViewShaderParametersBuffer, 0);
                    shaderArguments.BindTextureSRV(1, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(sceneColorInputTexture)));
                    shaderArguments.BindTextureSRV(2, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(depthOfFieldCoCTexture)));
                    shaderArguments.BindTextureUAV(3, RenderBackendTextureUAVDesc::Create(registry.GetRenderBackendTexture(depthOfFieldGatherTexture), 0));

                    shaderArguments.PushConstants(0, settings.postProcessingSettings.dofFarRegionBlurSize);

                    RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::DepthOfFieldGather);
                    commandList.Dispatch2D(
                        computeShader,
                        shaderArguments,
                        dispatchX,
        ...
        renderGraph.AddPass("DepthOfFieldPostfilter", RenderGraphPassFlags::Compute,
            [&](RenderGraphBuilder& builder)
            {
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                {
                    ...
                    RenderBackendShaderArguments shaderArguments = {};
                    shaderArguments.BindBuffer(0, sceneViewShaderParametersBuffer, 0);
                    shaderArguments.BindTextureSRV(1, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(depthOfFieldGatherTexture)));
                    shaderArguments.BindTextureSRV(2, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(depthOfFieldCoCTexture)));
                    shaderArguments.BindTextureUAV(3, RenderBackendTextureUAVDesc::Create(registry.GetRenderBackendTexture(depthOfFieldPostfilterTexture), 0));

                    shaderArguments.PushConstants(0, settings.postProcessingSettings.dofFarRegionBlurSize);

                    RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::DepthOfFieldPostfilter);
                    commandList.Dispatch2D(
                        computeShader,
                        shaderArguments,
                        dispatchX,
        ...
        renderGraph.AddPass("DepthOfFieldRecombine", RenderGraphPassFlags::Compute,
            [&](RenderGraphBuilder& builder)
            {
                ...
                return [=](RenderGraphRegistry& registry, RenderBackendCommandList& commandList)
                {
                    ...
                    shaderArguments.BindBuffer(0, sceneViewShaderParametersBuffer, 0);
                    shaderArguments.BindTextureSRV(1, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(sceneColorTexture)));
                    shaderArguments.BindTextureSRV(2, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(depthOfFieldPostfilterTexture)));
                    shaderArguments.BindTextureSRV(3, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(depthOfFieldCoCTexture)));
                    shaderArguments.BindTextureUAV(4, RenderBackendTextureUAVDesc::Create(registry.GetRenderBackendTexture(depthOfFieldOutputTexture), 0));

                    RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::DepthOfFieldRecombine);
                    commandList.Dispatch2D(
                        computeShader,
                        shaderArguments,
                        dispatchX,
