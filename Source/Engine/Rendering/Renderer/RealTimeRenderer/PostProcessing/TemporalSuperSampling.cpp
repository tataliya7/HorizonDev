        ...
        RenderGraphTextureHandle motionVectorTexture)
    {
        ...
        renderGraph.AddPass("TemporalSuperSampling", RenderGraphPassFlags::Compute,
            [&](RenderGraphBuilder& builder)
            {
                ...
                {
                    uint32 dispatchX = Math::CeilDiv(targetResolutionX, 8);
                    uint32 dispatchY = Math::CeilDiv(targetResolutionY, 8);
                    ...
                    shaderArguments.BindTextureSRV(3, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(motionVectorTexture)));
                    shaderArguments.BindTextureSRV(4, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(historySceneDepth)));
                    shaderArguments.BindTextureSRV(5, RenderBackendTextureSRVDesc::Create(registry.GetRenderBackendTexture(historyTemporalSuperSamplingTexture)));
                    shaderArguments.BindTextureUAV(6, RenderBackendTextureUAVDesc::Create(registry.GetRenderBackendTexture(outputTexture), 0));
                    shaderArguments.PushConstants(0, reset);

                    RenderBackendShaderHandle computeShader = shaderLibrary->GetShaderHandle((uint32)RealTimeRendererShaderPiplineID::TemporalSuperSampling);
