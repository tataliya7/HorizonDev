...
namespace HE
{
    ...
    {
        RenderBackendShaderHandle downsampleCubemapCS = shaderLibrary->GetShaderHandle((uint32)ShaderPipelineID::DownsampleCubemap);

        for (uint32 mipLevel = 1; mipLevel < numMipLevels; mipLevel++)
        {
            RenderBackendBarrier transitions[] =
            {
                RenderBackendBarrier(cubemap, RenderBackendTextureSubresourceRange(mipLevel - 1, 1, 0, RENDER_BACKEND_REMAINING_ARRAY_LAYERS), RenderBackendResourceState::UnorderedAccess, RenderBackendResourceState::ShaderResource),
                RenderBackendBarrier(cubemap, RenderBackendTextureSubresourceRange(mipLevel, 1, 0, RENDER_BACKEND_REMAINING_ARRAY_LAYERS), RenderBackendResourceState::Undefined, RenderBackendResourceState::UnorderedAccess)
            };
            commandList.Transitions(transitions, 2);

            uint32 dispatchX = Math::CeilDiv(1 << (numMipLevels - mipLevel - 1), 8);
            uint32 dispatchY = Math::CeilDiv(1 << (numMipLevels - mipLevel - 1), 8);
            uint32 dispatchZ = 1;
        ...
        RenderBackendBarrier transition = RenderBackendBarrier(cubemap, RenderBackendTextureSubresourceRange(numMipLevels - 1, RENDER_BACKEND_REMAINING_MIP_LEVELS, 0, RENDER_BACKEND_REMAINING_ARRAY_LAYERS), RenderBackendResourceState::UnorderedAccess, RenderBackendResourceState::ShaderResource);
    ...
    {
        RenderBackendShaderHandle computeEnvironmentIrradianceCS = shaderLibrary->GetShaderHandle((uint32)ShaderPipelineID::ComputeEnvironmentIrradiance);
        ...
        uint32 dispatchX = Math::CeilDiv(GIrradianceEnvironmentMapSize, 8);
        uint32 dispatchY = Math::CeilDiv(GIrradianceEnvironmentMapSize, 8);
        uint32 dispatchZ = 1;
    ...
    void ComputeEnvironmentIrradianceSH(ShaderLibrary_Deprecated* shaderLibrary, RenderBackendCommandList& commandList, RenderBackendTextureHandle environmentMap, uint32 environmentMapSize, RenderBackendBufferHandle irradianceEnvironmentMapSH)
    {
        ...
        RenderBackendShaderHandle computeEnvironmentIrradianceCS = shaderLibrary->GetShaderHandle((uint32)ShaderPipelineID::ComputeEnvironmentIrradianceSH);

        RenderBackendShaderArguments shaderArguments = {};
        shaderArguments.BindTextureSRV(0, RenderBackendTextureSRVDesc::Create(environmentMap));
        shaderArguments.BindBuffer(1, irradianceEnvironmentMapSH, 0);
    ...
    {
        RenderBackendShaderHandle filterEnvironmentMapCS = shaderLibrary->GetShaderHandle((uint32)ShaderPipelineID::FilterEnvironmentMap);

        RenderBackendBarrier transition(filteredEnvironmentMap, RenderBackendTextureSubresourceRange(0, RENDER_BACKEND_REMAINING_MIP_LEVELS, 0, RENDER_BACKEND_REMAINING_ARRAY_LAYERS), RenderBackendResourceState::Undefined, RenderBackendResourceState::UnorderedAccess);
        commandList.Transitions(&transition, 1);
        ...
        transition = RenderBackendBarrier(filteredEnvironmentMap, RenderBackendTextureSubresourceRange(0, RENDER_BACKEND_REMAINING_MIP_LEVELS, 0, RENDER_BACKEND_REMAINING_ARRAY_LAYERS), RenderBackendResourceState::UnorderedAccess, RenderBackendResourceState::ShaderResource);
        commandList.Transitions(&transition, 1);
    }
    ...
    {
        RenderBackendShaderHandle equirectangularToCubemapCS = shaderLibrary->GetShaderHandle((uint32)ShaderPipelineID::EquirectangularToCubemap);
        ...
        uint32 dispatchX = Math::CeilDiv(cubemapSize, 8);
        uint32 dispatchY = Math::CeilDiv(cubemapSize, 8);
        uint32 dispatchZ = 1;
